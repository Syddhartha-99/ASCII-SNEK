...
int main()
{
	...
	while (1)
	{
		...
		while (!bDead)
		{
			...
			wsprintf(&screen[nScreenWidth + 5], L"ASCII SNEK               Score: %d", nScore);

			//Draw snake body
			for (auto s : snake)
				screen[s.y * nScreenWidth + s.x] = bDead ? L'+' : L'O';
			//Draw snake head
			screen[snake.front().y * nScreenWidth + snake.front().x] = bDead ? L'@' : L'X';
		...
		while ((0x8000 & GetAsyncKeyState((unsigned)('\x20'))) == 0);
	}
